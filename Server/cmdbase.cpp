...
string CmdBase::getResMsg(int resCode)
{
	...
	case RUNERR_MIS_PASSWORD:
		resMsg = "连接机器许可号码是不正确的错误";
		break;
	case RUNERR_MEMORYOVER:
		resMsg = "机器上不足存储器的错误";
		break;
	case RUNERR_DATADOUBLE:
		resMsg = "要登记的号码已经在机器数据里存在的错误";
		break;
	case RUNERR_MANAGEROVER:
		resMsg = "机器里没有空间，未能登记管理者.数据的错误";
		break;
	case RUNERR_FPDATAVERSION:
		resMsg = "要用的指纹数据的版本不正确的错误";
		break;
	default:
		resMsg = "未知";
		break;
	}
	return formartMsg(resCode,resMsg);
}

string CmdBase::formartMsg(int code,string msg)
{
	return "["+i2str(code)+"]"+msg;
}